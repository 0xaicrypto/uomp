...
>
> 重要前提：
> - **CLI 和 SDK 是 UOMP 规范的通用实现**，股票分析只是第一个具体应用场景。
> - 本设计文档中的股票相关示例，是为了验证通用设计在真实场景中的可用性；所有命令、API 和数据结构都应能直接迁移到其他类型的 Agent。
>
> 本设计同时面向两个角色：
...
> 通用导入格式已在 [UOMP Spec §12 Memory Import Format](/spec/) 中定义。本节是该规范在股票分析场景下的**应用补充**，包括推荐字段别名、敏感度默认值和示例数据格式。其他 Agent 场景应遵循同样的通用规范，只替换字段别名和默认值即可。
